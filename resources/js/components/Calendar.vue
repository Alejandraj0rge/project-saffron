        ...
        <div class="card" style="display:flex;justify-content:start;margin:10px">
            ...
            <div style="display: flex;padding:20px" v-for="(event, index) in eventList">
                <div style="display: flex;flex-direction:column;justify-content:space-between">
                    <div style="font-size:small">{{new Date(event.start_time).getHours() + ":" + new Date(event.start_time).getMinutes()}}</div>
                    <div style="font-size:small">{{new Date(event.end_time).getHours() + ":" + new Date(event.start_time).getMinutes()}}</div>
                </div>
